...
#collect all the attributes common to the whole file
    for child in root:
        if child.tag == "progId":
            progId = child.text
#find all Order elements in the xml file, loop through them looking for Allocation elements
...
#print run stats
print("Save complete")
print("Found " + str(allocationCoumt) + " allocations in " + str(fileCount) + " files")
